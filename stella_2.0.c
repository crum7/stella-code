...
//コマンドでb(black list表示)がrよりも前に行われた場合の処理
void gets_2(char *argv[],int *blacklist);
void downloads_2(char *argv[],int *blacklist);
void sets_2(char *argv[],int *blacklist);
void regs_2(char *argv[],int *blacklist);
void starts_2(char *argv[],int *blacklist);
void files_2(char *argv[],int *blacklist);
void resources_2(char *argv[],int *blacklist);
void computers_2(char *argv[],int *blacklist);
void locals_2(char *argv[],int *blacklist);
void devices_2(char *argv[],int *blacklist);
void exes_2(char *argv[],int *blacklist);
void sleeps_2(char *argv[],int *blacklist);
void internets_2(char *argv[],int *blacklist);
void dlls_2(char *argv[],int *blacklist);
void others_2(char *argv[],int *blacklist);

//stella単独で実行された場合(引数となるマルウェアファイルがない場合)
void online_no(char *argv[]);
void hashes_no(char *argv[]);
void gets_no(char *argv[],int *blacklist);
void downloads_no(char *argv[],int *blacklist);
void sets_no(char *argv[],int *blacklist);
void regs_no(char *argv[],int *blacklist);
void starts_no(char *argv[],int *blacklist);
void files_no(char *argv[],int *blacklist);
void resources_no(char *argv[],int *blacklist);
void computers_no(char *argv[],int *blacklist);
void locals_no(char *argv[],int *blacklist);
void devices_no(char *argv[],int *blacklist);
void exes_no(char *argv[],int *blacklist);
void sleeps_no(char *argv[],int *blacklist);
void internets_no(char *argv[],int *blacklist);
void dlls_no(char *argv[],int *blacklist);
void others_no(char *argv[],int *blacklist);
void http_no(char *argv[],int *blacklist);

//stella単独で実行された場合(引数となるマルウェアファイルがない場合)かつ//コマンドでb(black list表示)がrよりも前に行われた場合の処理
void gets_2_no(char *argv[],int *blacklist);
void downloads_2_no(char *argv[],int *blacklist);
void sets_2_no(char *argv[],int *blacklist);
void regs_2_no(char *argv[],int *blacklist);
void starts_2_no(char *argv[],int *blacklist);
void files_2_no(char *argv[],int *blacklist);
void resources_2_no(char *argv[],int *blacklist);
void computers_2_no(char *argv[],int *blacklist);
void locals_2_no(char *argv[],int *blacklist);
void devices_2_no(char *argv[],int *blacklist);
void exes_2_no(char *argv[],int *blacklist);
void sleeps_2_no(char *argv[],int *blacklist);
void internets_2_no(char *argv[],int *blacklist);
void dlls_2_no(char *argv[],int *blacklist);
void others_2_no(char *argv[],int *blacklist);


void blacklists(int *blacklist);


//flossとstringの切り替えは、#define Strと#undef Strで切り替えてるから。そこチェック。あと、stella ->のとこの色を変えようとしても変えようがないから。ctrl+Shift+Lを使ってけ。コマンド足したくなったら、switch文いじれ。
//逆アセンブラの方に手を出すか。出てきた関数に関するまとめ作るか。資料まとめるか。スライド作るか。ポスター発表のポスター作るか。stellaとflossのパス通すの忘れるな。逆アセンブラツールとか言うのもいいかもな。動的解析は、めんどいし、virtualboxとの通信とかめんどくさい。
//これをwindows版にするのもいいかもしれないけど、初期のstella作るのとおんなじくらい。いやそれ以上めんどくさくなってくる。GUIにするのもせっかく対話形式にしたし、いいかな。なによりQt意味不。なんやねんあれ。c++もやらないとな。
//逆アセンブラって見る人間によってだいぶ変わるんかね。よくわからん。blacklistのグラフ化とかもいいかもね。うん。いいかもね。っていうか、#define最強じゃね。全部#defineでやったら喘ぎ声だけで、hello worldとか絶対行けるだろ。
//コードきもそう。このコードもだいぶキモいけど、これをきれいにする気力はない。flossとstringで分けなくてよくなったことだけでも誰かに褒めてほしいわ。stellaのロゴでもつくるかね
//あと、オプションつけて実行しようとするだろうけど、どんなにやっても無理だから諦めろ。絶対。っていうか、この対話形式だいぶ気に入ってるから。いまさら、オプションとかどうでもいいわ。/usr/local/binにおけばパス通るから、全部そこにつっこんどけ。
...
int main(int argc, char *argv[]){
    ...
    //stella単独で実行されている場合（マルウェアの設定を求める)
    if(argv[1]==NULL){

        system("clear");

        //ascii art
        system("figlet -f slant stella");
        printf("https://github.com/crum7/Stella\n\n");

        printf("解析対象を入力してください。\n");
        system("pwd");
        printf("\n");
        system("ls");
        printf("\n\nstella ->");
...
do{
    ...
    switch(c){
        ...
        case 'B':
            ...
            #define Str "strings"
            if(i==0){
                gets_2(argv,blacklist);
                downloads_2(argv,blacklist);
                sets_2(argv,blacklist);
                regs_2(argv,blacklist);
                starts_2(argv,blacklist);
                files_2(argv,blacklist);
                resources_2(argv,blacklist);
                computers_2(argv,blacklist);
                locals_2(argv,blacklist);
                devices_2(argv,blacklist);
                exes_2(argv,blacklist);
                sleeps_2(argv,blacklist);
                internets_2(argv,blacklist);
                dlls_2(argv,blacklist);
                others_2(argv,blacklist);
...
void commands_no(char *argv[512]){
...
do{
    ...
    switch(c){
        ...
        case 'R':
            ...
            #define Str "strings"
            i++;
            gets_no(argv,blacklist);
            downloads_no(argv,blacklist);
            sets_no(argv,blacklist);
            regs_no(argv,blacklist);
            starts_no(argv,blacklist);
            files_no(argv,blacklist);
            resources_no(argv,blacklist);
            computers_no(argv,blacklist);
            locals_no(argv,blacklist);
            devices_no(argv,blacklist);
            exes_no(argv,blacklist);
            sleeps_no(argv,blacklist);
            internets_no(argv,blacklist);
            dlls_no(argv,blacklist);
            others_no(argv,blacklist);
            http_no(argv,blacklist);
            printf("\nstella ->");
        break;
        ...
        case 'B':
            ...
            #define Str "strings"
            if(i==0){
                gets_2_no(argv,blacklist);
                downloads_2_no(argv,blacklist);
                sets_2_no(argv,blacklist);
                regs_2_no(argv,blacklist);
                starts_2_no(argv,blacklist);
                files_2_no(argv,blacklist);
                resources_2_no(argv,blacklist);
                computers_2_no(argv,blacklist);
                locals_2_no(argv,blacklist);
                devices_2_no(argv,blacklist);
                exes_2_no(argv,blacklist);
                sleeps_2_no(argv,blacklist);
                internets_2_no(argv,blacklist);
                dlls_2_no(argv,blacklist);
                others_2_no(argv,blacklist);
                blacklists(blacklist);
            }else{
                blacklists(blacklist);
            }
